...
export const isTokenExpiringSoon = async (data: ValidatedJWT): Promise<boolean> => {
...
export const combinePages = (pageGroups: PageGroup[], pages: Page[]): (PageGroup | Page)[] => {
...
export const sortGroupAndPage = (filteredGroups: PageGroup[], filteredPages: Page[]): (PageGroup | Page)[] => {
  const combinedPages: (PageGroup | Page)[] = [...filteredGroups, ...filteredPages];
...
export function getClosestVersion (cloneData: Documentation[]): ClosestVersion | null {
  ...
  return cloneData.reduce((closest: ClosestVersion | null, obj: Documentation) => {
    ...
    if (
      !closest ||
      timeDifference < Math.abs(now.getTime() - new Date(closest.createdAt).getTime())
    ) {
      return {
        id: obj.id,
        version: obj.version,
...
export function getVersion (cloneData: Documentation[], versionId: number): ClosestVersion | null {
  const matchingObj = cloneData?.find((obj: Documentation) => obj.id === versionId);
...
export const validateFormData = (formData: CreateDocumentFormData): ValidationResult => {
  ...
  const urlFields: (keyof CreateDocumentFormData)[] = ['favicon', 'navImageDark', 'navImage', 'metaImage'];
...
export const validateCommunityFields = (socialPlatformField: SocialPlatformField[], moreField: MoreField[]): { status: boolean; message: string } => {
  const errors = {
    status: false,
    message: ''
  };
...
export const landingPageValidate = (data: LandingPage): { status: boolean; message: string } => {
  const checks = [
    {
    ...
    {
      condition: !isValidURL(data.ctaButtonText.ctaButtonLink) && !isValidBaseURL(data.ctaButtonText.ctaButtonLink),
